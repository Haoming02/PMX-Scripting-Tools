...
def my_pack(fmt:str, args_in: Any) -> bytearray:
	...
	try:
		...
		MY_PRINT_FUNC("error in my_pack(fmt, args_in)")
		MY_PRINT_FUNC("fmt=", fmt, "args_in=", args_in)
...
def my_string_pack(S: str, L=None) -> bytearray:
	...
	try:
		...
		MY_PRINT_FUNC("error in my_string_pack(S,L)")
		MY_PRINT_FUNC("S=", S, "L=", L)
		MY_PRINT_FUNC(e.__class__.__name__, e)
		raise
...
def my_unpack(fmt:str, data:bytearray) -> Any:
	...
	try:
		...
		MY_PRINT_FUNC("error in my_unpack(fmt, data)")
		MY_PRINT_FUNC("fmt=",fmt,"data=","really big!","bytepos=", UNPACKER_READFROM_BYTE)
		MY_PRINT_FUNC(e.__class__.__name__, e)
		raise
	...
	for i in range(len(retme)):
		...
		if isinstance(foo, float):
			...
			if math.isinf(foo):
				...
				MY_PRINT_FUNC("Warning: found INF in place of float shortly before bytepos %d, replaced with +/- 999999.0" % UNPACKER_READFROM_BYTE)
...
def my_string_unpack(data: bytearray, L=None) -> str:
	...
	try:
		...
		MY_PRINT_FUNC("error in my_string_unpack(data,L)")
		MY_PRINT_FUNC("data=","really big!","L=",L)
		MY_PRINT_FUNC(e.__class__.__name__, e)
		raise
