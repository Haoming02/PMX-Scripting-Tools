	...
	for d, vert in enumerate(pmx.verts):
		...
		if round(sum(weightvals), 6) != 1.0:
			try:
				...
				weightvals = normalize_sum(weightvals)
...
def normalize_normals(pmx: pmxstruct.Pmx) -> Tuple[int,List[int]]:
	...
	for d,vert in enumerate(pmx.verts):
		...
		if vert.norm == [0, 0, 0]:
			...
			norm_L = euclidian_distance(vert.norm)
	...
	for d, (badvert_idx, badvert_faces) in enumerate(zip(normbad, normbad_linked_faces)):
		...
		for face_id in badvert_faces:
			...
			if not facenorm:
				...
				facenorm = cross_product(qr, qs)
				# then normalize the fresh normal
				try:
					facenorm = normalize_distance(facenorm)
				except ZeroDivisionError:
		...
		newnorm = normalize_distance(newnorm)
...
if __name__ == '__main__':
	core.RUN_WITH_TRACEBACK(main, helptext, '_weightfix', weight_cleanup)
